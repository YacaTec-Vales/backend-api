/**
 * @fileoverview Tests unitarios del `VerificadoresService`.
 *
 * Patron identico a coordinadores; cubre los scopes minimos.
 */
...
describe('VerificadoresService', () => {
  let service: VerificadoresService;
  ...
  beforeEach(async () => {
    ...
    service = module.get(VerificadoresService);
  });

  it('GERENTE_GENERAL crea verificador con branchId enviado', async () => {
    ...
    expect(userCreation.createInternalUser).toHaveBeenCalledWith(
      expect.objectContaining({ roleCode: 'VERIFICADOR', branchId: 'suc-1' }),
    );
  });
  ...
  it('ADMINISTRADOR no puede crear verificadores', async () => {
    await expect(
      service.create(
