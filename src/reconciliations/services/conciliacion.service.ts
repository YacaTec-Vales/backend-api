...
export class ConciliacionService {
  ...
  async procesarConciliacionAutomatica(
    ...
    await this.db.transaction(async (tx) => {
      ...
      const [batch] = await tx.insert(reconciliationBatches).values({
        uploadedBy,
        originalFileName,
        storagePath,
        sheetName: 'Hoja1',
      ...
      for (const movement of parsedMovements) {
        ...
        const relationList = await tx.select().from(relations)
        ...
        const [insertedMovement] = await tx.insert(bankMovements).values({
          batchId: batch.id,
          item: movement.item,
        ...
        if (relation) {
          ...
          const newStatus = newPaidCents >= Number(relation.totalToPayCents) ? 'LIQUIDADO' : 'PARCIAL';

          // Actualizar saldo y estado de la relación
          await tx.update(relations)
            .set({
          ...
          const [reconciliationRecord] = await tx.insert(reconciliations).values({
      ...
      await tx.update(reconciliationBatches)
        .set({
      ...
      this.logger.log(`Lote ${batch.id} procesado exitosamente. Movimientos totales: ${parsedMovements.length}, Cruzados: ${matchedCount}, Saldo huérfano (cents): ${branchCreditBalance}`);
    });
  }
  ...
  async procesarConciliacionManual(
    ...
    await this.db.transaction(async (tx) => {
      ...
      const [movement] = await tx.select().from(bankMovements)
        .where(eq(bankMovements.id, bankMovementId))
        .limit(1);
      
      if (!movement) throw new Error('Movimiento bancario no encontrado');
      if (movement.reconciliationId) throw new Error('El movimiento ya se encuentra conciliado');

      // Validar relación
      const [relation] = await tx.select().from(relations)
      ...
      await tx.execute(
        sql`SET LOCAL app.audit_action = 'CONCILIACION.MANUAL'`,
      );

      this.logger.log(`Conciliación manual ejecutada. Movimiento: ${bankMovementId}, Relación: ${relationId}`);
    });
  }
}
