/**
 * @fileoverview Servicio principal del modulo `cajeros`.
 *
 * Mismo patron que `coordinadores` y `verificadores`: alta + listado
 * de usuarios con rol `CAJERO`, reutilizando `UserCreationService`.
 *
 * @module cajeros
...
import type { CreateCajeroDto } from './dto/create-cajero.dto';
import type { ListCajerosQueryDto } from './dto/list-cajeros-query.dto';
import type { CreateInternalUserResult } from '../shared/user-creation/user-creation.service';

@Injectable()
export class CajerosService {
  private readonly logger = new Logger(CajerosService.name);
  ...
  async list(actor: RequestUser, query: ListCajerosQueryDto) {
  ...
  async findById(actor: RequestUser, id: string) {
    ...
    if (!user || user.roleCode !== 'CAJERO') {
      throw new NotFoundException({
        code: 'CAJERO.NOT_FOUND',
        message: 'cajero no encontrado',
      });
    }
    if (actor.role === 'GERENTE_SUCURSAL' && actor.branchId !== user.branchId) {
      throw new ForbiddenException({
        code: 'CAJERO.SCOPE_FORBIDDEN',
        message: 'no puedes ver cajeros de otra sucursal',
      });
    }
    return {
      id: user.id,
  ...
  private async resolveTargetBranch(
    ...
    if (actor.role === 'ADMINISTRADOR') {
      throw new ForbiddenException({
        code: 'CAJERO.SCOPE_FORBIDDEN',
        message: 'el administrador no puede crear cajeros',
      });
    }
    ...
    if (actor.role === 'GERENTE_SUCURSAL') {
      ...
      if (requestedBranchId && requestedBranchId !== actor.branchId) {
        throw new ForbiddenException({
          code: 'CAJERO.BRANCH_SCOPE_FORBIDDEN',
          message: 'no puedes crear cajeros en otra sucursal',
        });
      }
    ...
    throw new ForbiddenException({
      code: 'CAJERO.SCOPE_FORBIDDEN',
      message: 'no tienes permiso para crear cajeros',
    });
  }
